...
class SqliteSyncTracker(SqliteRelationalStore, AbstractSyncTracker):
    ...
    def cleanup_old_records(self, before: float) -> int:
        count = self.execute(
            f"DELETE FROM {self._table_artifacts} WHERE last_sync_time < ?", (before,)
        )
        self.commit()
    ...
    def sync_history(self, source_name: str) -> list[dict[str, Any]]:
        return self.query_raw(
            f"SELECT item_key, source_name, modified_time, last_sync_time FROM {self._table_artifacts} WHERE source_name=? ORDER BY last_sync_time DESC",
            (source_name,),
        )

    def sync_stats(self) -> dict[str, Any]:
        total_sources = self.query_raw(
            f"SELECT COUNT(*) AS cnt FROM {self._table_sources}"
        )[0]["cnt"]
        total_artifacts = self.query_raw(
            f"SELECT COUNT(*) AS cnt FROM {self._table_artifacts}"
        )[0]["cnt"]
        sources_with_conflicts = self.query_raw(
            f"SELECT COUNT(DISTINCT a.source_name) AS cnt FROM {self._table_artifacts} a JOIN {self._table_artifacts} b ON a.item_key = b.item_key AND a.source_name != b.source_name AND a.modified_time != b.modified_time"
        )[0]["cnt"]
        return {
    ...
    def _find_conflicts(self, source_name: str) -> list[ConflictItem]:
        rows = self.query_raw(
            f"SELECT DISTINCT a.item_key FROM {self._table_artifacts} a JOIN {self._table_artifacts} b ON a.item_key = b.item_key WHERE a.source_name = ? AND b.source_name != a.source_name AND a.modified_time != b.modified_time",
            (source_name,),
        )
