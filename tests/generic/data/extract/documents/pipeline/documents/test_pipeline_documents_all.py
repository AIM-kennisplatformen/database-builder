...
async def test_pipeline_documents_store_real(
    ...
    async with dataaccessor_qdrant as connection_qdrant:
...
@mark.anyio(scope="module")
async def test_pipeline_documents_load_real(
    configuration: Configuration,
    dataaccessor_qdrant: DataaccessorQdrant,
) -> None:
    # TODO: Encode as dependency, instead of re-running this test?
    await test_pipeline_documents_store_real(configuration=configuration, dataaccessor_qdrant=dataaccessor_qdrant)
